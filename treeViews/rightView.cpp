...
void printRightView(Node *root,int level,int &global){
    if (!root){
        return;
    }
    ...
    printRightView(root->right,level+1,global);
    printRightView(root->left,level+1,global);
}
